...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run Ec2api's test suite(s)"
  echo ""
...
function process_option {
  case "$1" in
    ...
    -f|--force) let force=1;;
    -u|--update) update=1;;
    --unittests-only) noseopts="$noseopts --exclude-dir=ec2api/tests/functional";;
    -c|--coverage) noseopts="$noseopts --with-coverage --cover-package=ec2api";;
    -*) noseopts="$noseopts $1";;
    *) noseargs="$noseargs $1"
  esac
}
